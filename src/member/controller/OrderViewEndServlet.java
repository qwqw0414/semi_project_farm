...
public class OrderViewEndServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(year==0 && month==0) {
			pageBar="";
			totalContent = new MemberService().orderTotalContent(memberId);	
			...
			pageBar="";
			totalContent = new MemberService().ordertotalContentByDate(memberId, year, month);
			int totalPage = (int) Math.ceil((double)totalContent/numPerPage);
			...
			//2.pageNo
				...
				while(pageNo<=pageEnd&&pageNo<=totalPage) {
					...
					if(cPage==pageNo) {
						...
						pageBar += "<li class='page-item'><a class='page-link' href='"+request.getContextPath()+"/member/orderViewEnd?memberId="+memberId+"&year="+year+"&month="+month+"&cPage="+pageNo+"'>"+pageNo+"</a></li>";
					}
					pageNo++;
				}
			//3. 다음
				if(pageNo<totalPage) {
					pageBar += "<li class='page-item'><a class='page-link' href='"+request.getContextPath()+"/member/orderViewEnd?memberId="+memberId+"&year="+year+"&month="+month+"&cPage="+pageNo+"'>≫</a></li>";
				}
